...
/* Consulta para total de registro filtrados */
$sqlTotal = "SELECT count($id) FROM $table WHERE rol_fk = '4'";
...
if ($num_rows > 0) {
    while ($row = $resultado->fetch_assoc()) {
        if ($row['rol_fk']==4){
            ...
            $output['data'] .= '<td><a class="btn btn-warning btn-sm" href="abogado_crear_caso.php?id1=' . $row['identificacion'] . '">Crear Caso</a></td>';
            $output['data'] .= '</tr>';
        }
    }
} else {
